...
microglia_genes <- read.delim("../microglia_genes_3fold_enrich.tsv")
names(microglia_genes)[names(microglia_genes) == 'GeneName'] <- 'gene_name'
```
...
# Remove Fc_34 sample since it's an outlier
metadata <- subset(metadata, Short.ID != "F_34")
...
# Save dge R object
```{r rOjects}
saveRDS(dge, file = "../rObjects/dge.raw.exclude_F_34.rds")
...
keepMicroglia <- dge$genes$gene_name %in% microglia_genes$gene_name
dge <- dge[keepMicroglia,,keep.lib.sizes = FALSE]
dim(dge)

rm(keepProtein, removeMT, keepMicroglia)
```
...
```{r filter}
keep.expr <- filterByExpr(dge, group = dge$samples$Treatment, min.count = 10)
dim(dge)
...
treat_colors <- c(treatment_colors)[dge.filtered.norm$samples$Treatment]

par(bg = 'white')
plotMDS(
  lcpm,
  top = 100, 
  labels = dge.filtered.norm$samples$Short.ID,
  cex = .8, 
  dim.plot = c(1,2), 
  plot = TRUE, 
  col = treat_colors,
...
```{r JSD_heatmap}
# JDS
data <- JSD(t(edgeR::cpm(dge.filtered.norm$counts)), est.prob = "empirical")
names <- dge.filtered.norm$samples$Short.ID
colnames(data) <- names
rownames(data) <- names
round.data <- round(data, digits = 3) # round 3 decimal places
...
write.table(cpm, "../results/all_samples/counts/cpm.txt", quote = FALSE, sep = "\t")
write.table(lcpm, "../results/all_samples/counts/lcpm.txt", quote = FALSE, sep = "\t")
```

# Save dge.filtered.norm R object
```{r}
saveRDS(dge.filtered.norm, file = "../rObjects/dge.filtered.norm.exclude_F_34.rds")
```

# Add biomarker expression to dge metadata 
```{r biomarker_expression, eval = FALSE}
...
```{r add_scale_data_to_dge, eval = FALSE}
...
form <- ~ Treatment + Age.at.harvest + Flowcell + Lane + (1|Short.ID)
...
```{r varpart}
form_varPart <- ~ (1|Treatment) + Age.at.harvest + (1|Lane)
varPart <- fitExtractVarPartModel(voomCounts, form_varPart, metadata)
vp <- sortCols(varPart)
```
...
```{r design}
design <-
  model.matrix(~ 0 + Treatment + Age.at.harvest + Lane,
    dge.filtered.norm$samples
  )

colnames(design) <- c("Adu", "Fc", "IgG", "Age", "Lane3",  "Lane4")
```
...
```{r voom_BIC}
form <- (~ 0 + Treatment + Age.at.harvest + Lane)
counts = DGEList(counts = dge.filtered.norm$counts)
...
fit <- dream(voom_cov, form, metadata)
...
veBayesFit <- variancePartition::eBayes(vfit, trend = TRUE, robust=TRUE)
...
genes_relevant <- subset(genes_relevant, gene_id %in% dge.filtered.norm$genes$gene_id)
aok  <-  topTable(veBayesFit, 
    coef = 2,  
    n = Inf, 
    p.value = 1,
    lfc = 0, 
...
# Save objects
```{r save_voom}
saveRDS(veBayesFit, file = "../rObjects/veBayesFit.rds")
saveRDS(voomCounts, file = "../rObjects/voomCountsMatrix.rds")
```
...
pcaWithMetadata <- merge(dim1_10, metadata, by = "Short.ID", all = TRUE)
...
for (i in allComparisons) {
  ...
  if (num != 0) {
    ...
    hadjpval <- (-log10(max(data$P.Value[data$adj.P.Val < 0.3],
                            na.rm = TRUE)))
    negFC <- c(-1)
    posFC <- c(1) 
    i <- gsub("_vs_", " vs ", i)
